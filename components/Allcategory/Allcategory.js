...
const Allcategory = () => {
  ...
  //       method: "POST",
  //       url: "/api/product/supercat/price/sortslug",
  //       data: {
  ...
  // Start get product by category

  const getProductsByCategory = async ({ paginationNumber, sortKey }) => {
    setProducts(undefined);
    ...
    try {
      const response = await axios({
        method: "POST",
        url: "/api/product/supercatwebslugfilter/",
        data: {
          supercategorySlug: superCat,
          color: colors,
  ...
  const increment = () => {
    if (count < pagesArray.length) {
      getProductsByCategory({ paginationNumber: count + 1 });
    } else {
      return;
    }
  };
  ...
  useEffect(() => {
    getProductsByCategory({ paginationNumber: 0 });
    setProducts(undefined);
  }, [superCat, colors]);
        ...
        <div className="dravo-grid-sorting row align-items-center">
          ...
          <div className="col-lg-6 col-md-6 ordering">
            <div className="select-box">
              ...
              <select
                onChange={(e) => {
                  getProductsByCategory({ sortKey: e.target.value });
                }}
              >
                    ...
                    <div className="col-lg-4 col-md-4 col-sm-4" key={index}>
                      <div className="single-products-box">
                        <Link href={`/product/gold/${products[index].slug}`}>
                          <a className="d-block">
                            {/* <img src={data?.image} alt={data && data.name} /> */}
                            {/* <LazyLoadImage
                              height="500"
                              src={data?.image}
                              width="100%"
                    ...
                    {pagesArray?.map((data, index) => {
                      return (
                        <a
                          ...
                          onClick={() => {
                            getProductsByCategory({ paginationNumber: index });
                            window.scrollTo({ top: 0, behavior: "smooth" });
                          }}
                          key={index}
